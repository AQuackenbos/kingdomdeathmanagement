...
export default {
  ...
  methods: {
    ...
    saveField(field, refName) {
      ...
      db.collection(`campaigns/${this.campaign.id}/survivors`).doc(this.currentSurvivor.id).update(updateObj)
        .then(() => loading.close())
    },
    ...
    deleteSurvivor() {
      this.$buefy.dialog.confirm({
        ...
        onConfirm: () => {
          this.setLoading(true)
          db.collection(`campaigns/${this.campaign.id}/survivors`).doc(this.currentSurvivor.id).delete().then(() => {
